...
def _resolve_prompt(data: str | list[dict[str, Any]], cached: CachedModel) -> str:
    """Standardize input to a context-clipped prompt string."""
    prompt = data if isinstance(data, str) else preprocess.build_prompt_from_messages(data)
    return preprocess.truncate_to_context(prompt, cached.entry.context_length * 4)
